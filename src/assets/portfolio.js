jQuery(document).ready( 
	function($){
		...
		const $portfolioRootContainer = $('#' + ajax_object.prefix + '-portfolio-list');
		// fetch data from api
			...
			$.ajax(
				{
					...
					success: function(response){
						...
						if(response.success){
							...
							devsharkTotalPageNavigationHandler(response.data.data["total-page"] , $('.devshark_post_pagination'))
		...
		*/
		...
		function devsharkTotalPageNavigationHandler($totalPages , $container , target = 1 , $showPages = 10){
			if($totalPages <= 1){
				return ;
			}
			...
			for(let c = 0 ; multifyTotalPage > c ; c++){
				...
				$html += `<div class="devshark-pagination-wrap ${count == 1 ? "active" : ""} " > `;
				if(multifyTotalPage !== count){
					...
					for(let i = 0 ; ($totalPages <= $showPages ? $totalPages : (minusValueOfPoints * $showPages)) > i ; i++ ){
						let valueOfPagination = countItemOfPagination+i ;
						$html += `<button 
						class="devshark-pagination-item ${ i + 1 == 1 ? 'active' : ""}" 
						value="${valueOfPagination}" >${valueOfPagination}</button>` ;
		...
		function nextButtonHandler(totalPages, targetPages, $container) {
			...
			$btnNextBtn.on("click", function() {
				...
				paginationItemClickHandler($(".devshark-pagination-item") , $container , nextPage);
				handlePaginationWrapChange($btnNextBtn , totalPages , $container , 10 , "top" )
		...
		function prevButtonHandler(totalPages, targetPages, $container ) {
			...
			$btnPrevBtn.on("click", function() {
				...
				paginationItemClickHandler($(".devshark-pagination-item") , $container , prevPage);
				handlePaginationWrapChange($btnPrevBtn , totalPages , $container , 10 , "bottom")
				margeAjaxRequestHandler(currentPage)
			});
